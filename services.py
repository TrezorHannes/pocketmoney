...
async def _execute_plan_inner(
    ...
    for item, sats in item_conversions:
        ...
        try:
            ...
            if dest_wallet:
                ...
                succeeded_labels.append(item.label)
    ...
    # Stage 4: Determine execution status
    # PARTIAL = some succeeded, some failed — user must NOT blindly re-run all
    # to avoid double-paying recipients that already received funds.
    any_succeeded = len(succeeded_labels) > 0
    any_failed = len(failed_labels) > 0

    if not any_failed:
        exec_status = ExecutionStatus.SUCCESS.value
        error_summary = None
    elif any_succeeded and any_failed:
        exec_status = ExecutionStatus.PARTIAL.value
        error_summary = (
            f"Partial failure: {len(succeeded_labels)}/{len(item_conversions)} paid. "
            f"Succeeded: {', '.join(succeeded_labels)}. "
            f"Failed: {', '.join(failed_labels)}. "
            f"First error: {first_error}. "
            f"⚠️ Check execution details before retrying — do not re-run all recipients."
        )
        logger.warning(f"Plan {plan.name} partially completed: {error_summary}")
    else:
    ...
    # Stage 6: Check low-balance threshold warning after payments
    ...
    should_notify = (
        (exec_status == ExecutionStatus.SUCCESS.value and settings.notify_on_success)
        or (exec_status in (ExecutionStatus.FAILED.value, ExecutionStatus.PARTIAL.value) and settings.notify_on_failure)
    )
    if should_notify:
